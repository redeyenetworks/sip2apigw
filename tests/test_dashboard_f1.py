...
# Unambiguously historical (never == "today"), so is_live is deterministically False.
HIST = "2020-06-15"
...
class TestF1CallTable:
    def test_selected_date_queries_that_local_day_window(self, tmp_path):
        ...
        start, end = _local_day_window(HIST, NY)
    ...
    def test_default_live_day_uses_today_page_not_between(self, tmp_path):
        # Default view = TODAY (live) -> existing today-only page path, NOT the
        # historical between-window query.
        db, c = _make(tmp_path, between_rows=[_row(3, 1751556600.0)],
                      page_rows=([_row(9, 1751470200.0)], 1, 1))
        r = c.get("/")
        assert r.status_code == 200
        db.get_calls_page.assert_awaited()
        db.get_calls_between.assert_not_awaited()
    ...
    def test_between_failure_falls_back_no_500(self, tmp_path):
        ...
        r = c.get("/?logdate=" + HIST)   # historical -> hits between -> raises -> fallback
        assert r.status_code == 200
        db.get_calls_page.assert_awaited()
